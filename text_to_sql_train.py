...
class TextToSQLConfig:
    ...
    def __init__(self):
        ...
        self.base_model_name = "Salesforce/codet5p-770m"
        ...
        self.lora_r = 8  # Rank of the adaptation matrices (higher = more parameters)
        # Have reduced the r to 8 given the Max of 1000 models.
        self.lora_alpha = 8  # Scaling factor for LoRA weights (typically 2x the rank)
        # Have reduced the alpha to 8 ( = rank) due to limited resources
        ...
        # to optimize for less resources, I will only be using q_proj and v_proj
        ...
        # Commenting this out as unfortunately my CUDA GPU is only 4GB
        # self.nvidia_optimizations = {
        #     "larger_batch_size": 1,       # Leverage GPU memory for larger batches
        ...
        # As my GPU is extremely small, with 4GB VRAM, need to make the settings more friendly.
        # hence disabiling most of the high power optimizations.
        self.nvidia_optimizations = {
            "larger_batch_size": 1,       # Leverage GPU memory for larger batches
            "use_bf16": False,             # Brain float 16 for better precision than fp16
            "enable_grad_checkpointing": False,  # Save memory for larger models
            "faster_evaluation": 1000,     # More frequent evaluation since GPU can handle it
            "use_flash_attention": False,  # Advanced attention optimization (if available)
        }
...
class ModelTrainer:
    ...
    def setup_model_and_tokenizer(self, device_type="auto"):
        ...
        try:
            ...
            device_map = "cuda" #"auto"          # Default device mapping
            ...
            if device_type == "mps":
                ...
                device_map = None #"auto"          # Let Transformers handle GPU memory allocation
    ...
    try:
        ...
        if args.mode in ["train", "full"]:
            ...
            if forced_device:
                ...
                if device_type == "cpu":
                    trainer.config.apply_cpu_optimizations()
                elif device_type == "mps":
                    trainer.config.apply_mac_optimizations()
                elif device_type == "cuda":
                    trainer.config.apply_nvidia_optimizations()
...
def test():
    ...
    if args.mode in ["test", "full"]:
        ...
        # Test deployed Ollama model
        deployer = OllamaDeployer(config)
        results = deployer.test_ollama_model()
